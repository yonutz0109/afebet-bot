...
function baseScoreFromOdd(odd,market){let s=Math.round((1/Number(odd))*100);if(market==="h2h")s-=4;if((market||"").includes("total"))s-=1;if((market||"").includes("spread"))s-=3;if(odd>=1.12&&odd<=1.45)s+=8;if(odd>1.65)s-=20;if(odd>1.90)s-=30;return Math.max(0,Math.min(95,s))}
...
async function findTeamId(name,key){if(teamCache.has(name))return teamCache.get(name);let data=await footballFetch(`/teams?search=${encodeURIComponent(name)}`,key);let id=data?.response?.[0]?.team?.id||null;teamCache.set(name,id);return id}
function formString(fixtures,teamId){let out=[];for(const f of fixtures||[]){let h=f.teams?.home?.id===teamId,a=f.teams?.away?.id===teamId;if(!h&&!a)continue;let hg=f.goals?.home,ag=f.goals?.away;if(hg==null||ag==null)continue;let gf=h?hg:ag,ga=h?ag:hg;out.push(gf>ga?"W":gf===ga?"D":"L");if(out.length>=5)break}return out.join("")||"n/a"}
function formPoints(form){return [...(form||"")].reduce((s,x)=>s+(x==="W"?3:x==="D"?1:0),0)}
async function getFootballStats(home,away,key){if(!key)return{boost:0,note:"API_FOOTBALL_KEY lipsește. Folosesc doar cotele.",homeForm:"n/a",awayForm:"n/a",h2hCount:0};let ck=home+"|"+away;if(statsCache.has(ck))return statsCache.get(ck);try{let homeId=await findTeamId(home,key),awayId=await findTeamId(away,key);if(!homeId||!awayId){let v={boost:0,note:"Nu am găsit echipele în API-FOOTBALL.",homeForm:"n/a",awayForm:"n/a",h2hCount:0};statsCache.set(ck,v);return v}let [hf,af,h2h]=await Promise.all([footballFetch(`/fixtures?team=${homeId}&last=5`,key),footballFetch(`/fixtures?team=${awayId}&last=5`,key),footballFetch(`/fixtures/headtohead?h2h=${homeId}-${awayId}&last=5`,key)]);let homeForm=formString(hf?.response,homeId),awayForm=formString(af?.response,awayId),h2hCount=h2h?.response?.length||0;let hp=formPoints(homeForm),ap=formPoints(awayForm),diff=ap-hp,boost=0;if(Math.abs(diff)>=6)boost+=3;else if(Math.abs(diff)>=3)boost+=2;if(h2hCount>=3)boost+=1;let v={boost,note:`Formă recentă: ${home} ${homeForm} (${hp}p), ${away} ${awayForm} (${ap}p). Bonus mic aplicat peste scorul din cote.`,homeForm,awayForm,h2hCount};statsCache.set(ck,v);return v}catch(e){let v={boost:0,note:"API-FOOTBALL nu a putut fi citit complet. Folosesc doar cotele.",homeForm:"n/a",awayForm:"n/a",h2hCount:0};statsCache.set(ck,v);return v}}
...
export default async function handler(req,res){
 ...
 try{
  ...
  for(const ev of (data||[])){
   ...
   for(const bookmaker of (ev.bookmakers||[])){
    for(const market of (bookmaker.markets||[])){
     const mk=market.key||"unknown",cat=category(mk);marketStats[cat]=(marketStats[cat]||0)+(market.outcomes||[]).length;
     for(const outcome of (market.outcomes||[])){
      outcomesChecked++;const odd=Number(outcome.price);if(!odd)continue;
      let score=Math.max(0,Math.min(95,baseScoreFromOdd(odd,mk)+Number(footballStats.boost||0)));
      const candidate={match:`${home} vs ${away}`,pick:pickLabel(outcome,mk,home,away),marketKey:mk,marketLabel:marketLabel(mk),odd:odd.toFixed(2),safeScore:score,startTime:roDate(ev.commence_time),bookmaker:bookmaker.title||"Bookmaker",footballStats,reason:score>=MIN_SAFE_SCORE?"Selecția trece pragul SafeBet, inclusiv ajustarea API-FOOTBALL.":`Nu recomand pariu: Safe Score ${score}/100 este sub pragul 75/100, dar acesta este cel mai bun găsit.`,rejectReason:`Safe Score ${score}/100 este sub pragul minim ${MIN_SAFE_SCORE}/100.`,verdict:score>=MIN_SAFE_SCORE?"JOACĂ":"CEL MAI BUN GĂSIT",rawScore:score};
  ...
  const report={eventsChecked,outcomesChecked,minSafeScore:MIN_SAFE_SCORE,scannedAt:roDate(new Date().toISOString()),marketStats,bestOverall:clean(bestOverall),firstAvailable:clean(firstAvailable),topOverall:topOverall.map(clean)};
