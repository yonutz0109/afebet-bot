...
export default async function handler(req,res){
 ...
 try{
   ...
   for(const bookmaker of (ev.bookmakers||[])){
    for(const market of (bookmaker.markets||[])){
     ...
     for(const outcome of (market.outcomes||[])){
      ...
      const candidate={match:`${home} vs ${away}`,pick:pickLabel(outcome,mk,home,away),marketKey:mk,marketLabel:marketLabel(mk),odd:odd.toFixed(2),safeScore:score,startTime:roDate(ev.commence_time),bookmaker:bookmaker.title||"Bookmaker",reason:score>=MIN_SAFE_SCORE?"Selecția trece pragul SafeBet.":`Nu recomand pariu: Safe Score ${score}/100 este sub pragul 75/100, dar acesta este cel mai bun găsit.`,rejectReason:`Safe Score ${score}/100 este sub pragul minim ${MIN_SAFE_SCORE}/100.`,verdict:score>=MIN_SAFE_SCORE?"JOACĂ":"CEL MAI BUN GĂSIT",rawScore:score};
  ...
  // NU întoarce niciodată tip:null dacă există măcar o selecție.
