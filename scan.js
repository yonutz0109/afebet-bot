...
async function getSoccerSports(key){const url=`${ODDS_BASE}/sports/?apiKey=${key}`;const r=await oddsFetch(url);if(!r.ok||!Array.isArray(r.data))return["soccer"];let sports=r.data.filter(s=>String(s.key||"").startsWith("soccer_")&&s.active!==false);let priority=["soccer","soccer_fifa_world_cup","soccer_uefa_european_championship","soccer_conmebol_copa_america","soccer_brazil_campeonato","soccer_brazil_serie_b","soccer_japan_j_league","soccer_japan_j_league_2","soccer_norway_eliteserien","soccer_sweden_allsvenskan","soccer_korea_kleague1","soccer_usa_mls","soccer_china_superleague","soccer_argentina_primera_division"];
let keys=[...new Set(["soccer",...priority,...sports.map(s=>s.key)])];return keys.slice(0,MAX_SPORTS_TO_SCAN)}
...
export default async function handler(req,res){
 ...
 try{
  ...
  const report={eventsChecked,outcomesChecked,minSafeScore:MIN_SAFE_SCORE,scannedAt:roDate(new Date().toISOString()),sportsScanned:sports.length,sportsUsed:sports,marketStats,timeStats,bestOverall:clean(bestOverall),firstAvailable:clean(firstAvailable),topOverall:topOverall.map(clean)};
