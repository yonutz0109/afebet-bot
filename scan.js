...
function timeInfo(iso){
 ...
 if(start && diffMin<=0 && diffMin>=-150)return{status:"LIVE ACUM",boost:3,key:"live"};
 if(start && diffMin>0 && diffMin<=60)return{status:"ÎNCEPE CURÂND",boost:2,key:"soon"};
...
async function findTeamId(name,key){
 ...
 let data=await footballFetch(`/teams?search=${encodeURIComponent(name)}`,key);
 let id=data?.response?.[0]?.team?.id||null;
...
function formString(fixtures,teamId){
 ...
 for(const f of fixtures||[]){
  let h=f.teams?.home?.id===teamId,a=f.teams?.away?.id===teamId;
  if(!h&&!a)continue;
  let hg=f.goals?.home,ag=f.goals?.away;
  if(hg==null||ag==null)continue;
  let gf=h?hg:ag,ga=h?ag:hg;
...
async function getFootballStats(home,away,key){
 ...
 try{
  let homeId=await findTeamId(home,key),awayId=await findTeamId(away,key);
  if(!homeId||!awayId){
   let v={boost:0,note:"Nu am găsit echipele în API-FOOTBALL. Folosesc doar cotele.",homeForm:"n/a",awayForm:"n/a",h2hCount:0};
   statsCache.set(ck,v);return v;
  }
  let [hf,af,h2h]=await Promise.all([
  ...
  let homeForm=formString(hf?.response,homeId),awayForm=formString(af?.response,awayId);
  let hp=formPoints(homeForm),ap=formPoints(awayForm),h2hCount=h2h?.response?.length||0;
  let boost=0,diff=Math.abs(hp-ap);
  if(diff>=6)boost+=3;else if(diff>=3)boost+=2;
  if(h2hCount>=3)boost+=1;
  let v={boost,homeForm,awayForm,h2hCount,note:`Formă recentă: ${home} ${homeForm} (${hp}p), ${away} ${awayForm} (${ap}p). Bonus API-FOOTBALL: +${boost}.`};
  statsCache.set(ck,v);return v;
 }catch(e){
  let v={boost:0,note:"API-FOOTBALL nu a putut fi citit complet. Folosesc doar cotele.",homeForm:"n/a",awayForm:"n/a",h2hCount:0};
  statsCache.set(ck,v);return v;
 }
}
...
export default async function handler(req,res){
 ...
 try{
  ...
  if(!r.ok)return res.status(500).json({error:data?.message||"The Odds API a returnat eroare.",help:"Verifică cheia sau piețele disponibile."});
  ...
  let marketStats={h2h:0,totals:0,spreads:0,other:0}, timeStats={live:0,soon:0,prematch:0};
